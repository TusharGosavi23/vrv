...
const Sidebar = () => {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed top-0 left-0 z-40 w-64 bg-gray-800 text-white h-screen transform transition-transform duration-200 ease-in-out md:relative md:w-20 md:flex-col flex flex-col
          ${isOpen ? "translate-x-0" : "-translate-x-full"} md:translate-x-0`}
      >
        <div className="flex items-center p-4 border-b border-gray-700 md:border-none md:justify-center">
        ...
        <nav className="flex-1 overflow-y-auto py-4 space-y-2">
          {menuItems.map((item) => (
            ...
            >
              <span className="inline-flex items-center justify-center w-8">{item.icon}</span>
